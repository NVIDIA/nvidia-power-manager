...
int flash_remote_mb_fpga_image(int bus, int image_sel, char *image,
  ...
  ret = std_send_i2c_cmd(fd, FPGA_READ, MB_REG_ADDR, write_buffer, read_buffer,
                         1, 2);
  if (ret)
    goto exit;
  if (debug_h) {
  ...
  write_buffer[5] = 0xff;
  write_buffer[6] = 0xff;
  if (image_sel == 1) { // MB FPGA image 1
    write_buffer[7] = 0xfc;
  } else if (image_sel == 0) { // MB FPGA image 0
    write_buffer[7] = 0xfb;
  } else {
    ret = -ERROR_IMAGE_SEL;
    goto exit;
  ...
  if (image_sel == 1) {
    ...
    ret =
        wait_until_not_busy(fd, DELAY_1000MS); // 2018.10.01 Jason change to 1 s
    if (ret)
      goto exit;
