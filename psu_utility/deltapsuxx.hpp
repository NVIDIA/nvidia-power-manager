...
class DeltaPsu : public I2c
{
  public:
    ...
    {
        int ret = 0;
        // 3. Unlock upgrade command with password... (Operation: 0xF0 0x0C
        // MCU_ID [ECD16010081]-ASCII PEC)  => retry 5
    ...
    int checkUpgradeModeStatus(uint8_t* write_buffer, uint8_t* read_buffer)
    {
        ...
        ret = I2c::transfer(fd, PSU_READ, slave, write_buffer, read_buffer, 1,
                            1);
        if (ret)
            return ret;
    ...
    int fwupdate(string imageFilename, int section)
    {
        ...
        VERBOSE << "PSU FW CRC16 Check: " << std::hex
                << static_cast<int>(fw_crc16) << endl;
        CRC16_Data = fw_crc16;
        ...
        Flag_SetUnlockBootflag =
            0; // Fail count for set "unlock" and "bootflag"
    SetUnlockBootflag:
        // 3. Unlock upgrade command with password... (Operation: 0xF0 0x0C
        // MCU_ID [ECD16010081]-ASCII PEC)  => retry 5
        ret = unlockDevice(write_buffer, read_buffer);
        ...
        if ((read_buffer[0] & bootStat) != bootStat) // Bit2=1? No!
        {
            ...
            else
            {
                ...
                cerr << __func__ << ":" << __LINE__ << "-"
                     << statusToString(ErrorStatus::ERROR_ENTER_BOOTLOADER)
                     << endl;
                goto exit;
            }
        }
    ...
    JumpToTransmit: // Jump to here after Skip Turn OFF, Unlock and Set Bootflag
        ...
        bytePerBlock =
            static_cast<int>((bytePerPage / bytePerPacket)); // 256 / 16
        ...
        for (i = 0; i < Page_Count; i++)
        {
            ...
            {
                /* 6. Transmit 16 x 16 Bytes into RAM (Operation: 0xF2 0x13
                 * MCU_ID Block_LO Block_HI Data[16] PEC) */
            ...
            RepeatTrans:                  // mv
                ...
                write_buffer[2] = mcuID;
                write_buffer[3] =
                    static_cast<uint8_t>((blockIndex)); // Block_LO
                write_buffer[4] = 0x00;                 // Block_HI

                uint8_t* block = static_cast<uint8_t*>(
                    malloc(bytePerPacket * sizeof(uint8_t)));
                ...
                {
                    ret = I2c::transfer(fd, PSU_WRITE, slave, write_buffer, 0,
                                        22, 0);
                    if (ret)
                        goto transferFail;
                    ...
                    ret = checkUpgradeModeStatus(write_buffer, read_buffer);
                    VERBOSE << "Status: " << std::hex
                            << static_cast<int>(read_buffer[0]) << endl;
                    if ((read_buffer[0] & TRANS_ERR) == TRANS_ERR)
                    {
                        cerr << __func__ << ":" << __LINE__ << "-"
                             << "#6 Programming 'Transmit' status failed: "
                             << TRANS_ERR << endl;
                        Flag_RepeatTrans++;
                        goto RepeatTrans;
                    }
                ...
                else
                {
                    ...
                    cerr << __func__ << ":" << __LINE__ << "-"
                         << statusToString(ErrorStatus::ERROR_TRANS_BLOCK)
                         << endl;
                    break;
                }
            }
            ...
            /* 7. Write RAM Data into FLASH (256 bytes) (Operation: 0xF3
             * 0x03(byte count) MCU_ID PageAddr_LO PageAddr_HI PEC) */
        ...
        RepeatWrite:
            ...
            {
                ret = I2c::transfer(fd, PSU_WRITE, slave, write_buffer, 0, 6,
                                    0);
                if (ret)
                {
                    Flag_RepeatWrite++;
    ...
    int fwversion()
    {
        ...
        if (ret)
            return ret;
        cout << " Version:Byte Count " << static_cast<int>(read_buffer[0])
             << endl;
